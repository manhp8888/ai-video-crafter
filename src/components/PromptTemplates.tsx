import { Film, Flame, ShoppingBag, Clapperboard, Wand2, Building2, User, Eye } from "lucide-react";
import type { PromptData } from "@/lib/prompt";

const templates: { name: string; icon: React.ReactNode; color: string; data: PromptData }[] = [
  {
    name: "TikTok Girl",
...
const PromptTemplates = ({ onSelect }: Props) => (
  <div className="space-y-3">
    <label className="text-sm font-semibold text-foreground">Mẫu có sẵn</label>
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
      {templates.map((t) => (
        <button
          key={t.name}
          onClick={() => onSelect(t.data)}
          className={`flex items-center gap-2.5 bg-gradient-to-br ${t.color} border rounded-xl px-3.5 py-3 text-xs font-semibold text-foreground hover:scale-[1.02] active:scale-[0.98] transition-all duration-150 text-left`}
        >
